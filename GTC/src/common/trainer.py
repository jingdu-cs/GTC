...
class Trainer(AbstractTrainer):
   ...
   and some other features helpful for model training and evaluation.
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None):
        ...
        for batch_idx, interaction in enumerate(train_data):
            ...
            if self._check_nan(loss):
                ...
                return loss, torch.tensor(0.0), step_count
    ...
    def _generate_train_loss_output(self, epoch_idx, s_time, e_time, losses, steps=None):
        train_loss_output = 'epoch %d training [time: %.2fs' % (epoch_idx, e_time - s_time)
        if steps is not None:
            train_loss_output += ', steps: %d' % steps
        train_loss_output += ', '
        if isinstance(losses, tuple):
    ...
    def fit(self, train_data, valid_data=None, test_data=None, saved=False, verbose=True):
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            train_loss, _, steps = self._train_epoch(train_data, epoch_idx)
            ...
            if (self.enable_epoch_visualization and 
                ...
                try:
                    self.logger.info(f"Tracking user-modality consistency for epoch {epoch_idx}")
                    self.model.track_user_modality_consistency(epoch_idx)
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                if (self.enable_epoch_visualization and 
                    ...
                    epoch_idx % self.visualization_interval == 0 and
                    hasattr(self.model, 'epoch_test_visualization')):
                    try:
                        self.logger.info(f"Performing validation visualization for epoch {epoch_idx}")
                        self.model.epoch_test_visualization(epoch_idx, is_validation=True)
                    except Exception as e:
                        self.logger.warning(f"Validation visualization failed: {e}")
                ...
                if (self.enable_epoch_visualization and 
                    ...
                    hasattr(self.model, 'epoch_test_visualization')):
                    try:
                        self.logger.info(f"Performing test visualization for epoch {epoch_idx}")
                        self.model.epoch_test_visualization(epoch_idx, is_validation=False)
                    except Exception as e:
                        self.logger.warning(f"Test visualization failed: {e}")
        ...
        if (self.enable_epoch_visualization and 
            ...
            try:
                self.logger.info("Generating final user-modality consistency analysis...")
                self.model.analyze_user_modality_trends()
            except Exception as e:
                self.logger.warning(f"Final user-modality analysis failed: {e}")
    ...
    def _show_steps_summary(self):
        ...
        total_steps = sum(self.epoch_steps_dict.values())
        avg_steps = total_steps / len(self.epoch_steps_dict)
        max_steps = max(self.epoch_steps_dict.values())
        min_steps = min(self.epoch_steps_dict.values())
        
        self.logger.info("=" * 50)
        self.logger.info("Training Steps Summary:")
        self.logger.info(f"Total epochs trained: {len(self.epoch_steps_dict)}")
        self.logger.info(f"Total training steps: {total_steps}")
        self.logger.info(f"Average steps per epoch: {avg_steps:.1f}")
